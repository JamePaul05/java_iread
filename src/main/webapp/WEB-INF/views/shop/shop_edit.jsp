            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form:form class="form-horizontal" commandName="shop" method="post"
                               action="${pageContext.request.contextPath}/admin/shop/save?type=edit" id="barForm">
...
<script>
    ...
    function init() {
        map = new BMap.Map("map", {minZoom:11,maxZoom:22});   // 创建Map实例,并说明最大缩放和最小缩放级别
//        map.enableScrollWheelZoom();   //启用滚轮放大缩小
       ...
       /* */
        ...
        map.addEventListener("click", selectPoint);//地图添加点击监听
        var point = new BMap.Point($("#lng").val(),$("#lat").val());//根据经纬度设置中心点
        map.centerAndZoom(point, 18);
        map.addOverlay(new BMap.Marker(point));    //添加标注
        myValue = $("#address").val();//获取地址
        ac = new BMap.Autocomplete({    //建立一个自动完成的对象
            "input" : "address",
            "location" : map
        });
        ac.setInputValue(myValue);
        setAcEventListener(ac);//设置搜索框监听器
    }
    ...
    function setAcEventListener(ac) {
